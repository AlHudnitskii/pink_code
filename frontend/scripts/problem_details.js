...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!checkAuth()) return;
    if (!problemId) {
        console.error("Problem ID not found in URL");
        return;
    }
...
function loadProblemDetails() {
    ...
    .then(data => {
        ...
        document.getElementById('problem-description').innerText = data.description;
        document.getElementById('problem-difficulty').innerText = data.difficulty;
        document.getElementById('likes').innerText = data.rates?.likes || 0;
        document.getElementById('dislikes').innerText = data.rates?.dislikes || 0;
        document.getElementById('count-solutions').innerText = data.count_solutions || 0;
    })
    .catch(error => console.error('Error fetching problem details:', error));
